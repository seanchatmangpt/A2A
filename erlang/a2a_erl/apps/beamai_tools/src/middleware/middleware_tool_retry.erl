%%%-------------------------------------------------------------------
%%% @doc 工具重试中间件
%%%
%%% 在工具调用失败时自动进行重试，支持多种退避策略：
%%% - 可配置最大重试次数
%%% - 支持指数退避、线性退避和固定延迟三种策略
%%% - 可自定义哪些错误类型允许重试
%%% - 支持自定义重试判断函数和重试回调
%%%
%%% 使用的钩子：
%%% - post_invocation: 检测工具执行结果，如果是错误则根据配置进行重试
%%%
%%% @end
%%%-------------------------------------------------------------------
-module(middleware_tool_retry).

-behaviour(beamai_middleware).

-export([init/1, post_invocation/2]).
-export([is_retryable/2, calculate_delay/2]).

%%====================================================================
%% 类型定义
%%====================================================================

%% @type backoff_config() :: map().
%% 退避策略配置类型：
%% - type: 退避类型（exponential 指数 | linear 线性 | constant 固定）
%% - initial_delay: 初始延迟时间（毫秒），正整数
%% - max_delay: 最大延迟时间（毫秒），正整数
%% - multiplier: 退避倍数（仅 exponential 和 linear 使用），可选
-type backoff_config() :: #{
    type := exponential | linear | constant,
    initial_delay := pos_integer(),
    max_delay := pos_integer(),
    multiplier => number()
}.

%%====================================================================
%% 中间件回调函数
%%====================================================================

%% @doc 初始化重试配置和退避策略。
%%
%% 根据传入的配置选项设置重试参数，包括最大重试次数、退避策略、
%% 可重试错误类型等。同时初始化一个空的重试追踪器，用于记录
%% 每个工具的重试次数。
%%
%% 配置参数说明：
%% - max_retries: 最大重试次数（默认 3）
%% - backoff: 退避策略配置映射（默认指数退避，初始 1s，最大 30s，倍数 2）
%% - retryable_errors: 可重试的错误类型列表，或 all 表示所有错误（默认 all）
%% - retry_fn: 自定义重试判断函数 fun(Error, RetryCount)，可选
%% - on_retry: 重试回调函数 fun(ToolName, Error, RetryCount, Delay)，可选
%% - enable_delay: 是否启用延迟等待（默认 true）
%%
%% @param Opts 配置选项映射
%% @returns 包含重试配置和空重试追踪器的初始状态映射
-spec init(map()) -> map().
init(Opts) ->
    DefaultBackoff = #{
        type => exponential,
        initial_delay => 1000,
        max_delay => 30000,
        multiplier => 2
    },
    #{
        max_retries => maps:get(max_retries, Opts, 3),
        backoff => maps:merge(DefaultBackoff, maps:get(backoff, Opts, #{})),
        retryable_errors => maps:get(retryable_errors, Opts, all),
        retry_fn => maps:get(retry_fn, Opts, undefined),
        on_retry => maps:get(on_retry, Opts, undefined),
        enable_delay => maps:get(enable_delay, Opts, true),
        %% 按工具名称追踪重试次数
        retry_tracker => #{}
    }.

%% @doc 检测错误并触发重试。
%%
%% 在工具执行完成后检查结果，如果结果为 {error, Error} 形式，
%% 则调用 maybe_retry 判断是否需要重试。如果结果正常则直接通过。
%%
%% @param FilterCtx 过滤器上下文，包含 result 键表示工具执行结果
%% @param MwState 当前中间件状态，包含重试配置和追踪器
%% @returns ok（不需要重试或结果正常时）或 {skip, RetryInfo}（触发重试时）
-spec post_invocation(map(), map()) -> beamai_middleware:middleware_result().
post_invocation(FilterCtx, MwState) ->
    case maps:get(result, FilterCtx, undefined) of
        {error, Error} ->
            maybe_retry(Error, FilterCtx, MwState);
        _ ->
            ok
    end.

%%====================================================================
%% 公共辅助函数
%%====================================================================

%% @doc 判断错误是否可重试。
%%
%% 根据中间件配置判断给定的错误是否允许重试：
%% - 如果配置了自定义 retry_fn，则调用该函数判断
%% - 否则使用默认的错误类型匹配逻辑
%%
%% @param Error 错误信息，可以是原子、元组或其他类型
%% @param MwState 中间件状态（需包含 retryable_errors 和 retry_fn）
%% @returns true 表示可以重试，false 表示不可重试
-spec is_retryable(term(), map()) -> boolean().
is_retryable(Error, #{retryable_errors := RetryableErrors, retry_fn := RetryFn}) ->
    case RetryFn of
        undefined ->
            is_retryable_default(Error, RetryableErrors);
        RetryFn when is_function(RetryFn, 2) ->
            try RetryFn(Error, 0) catch _:_ -> false end
    end.

%% @doc 计算退避延迟时间。
%%
%% 根据当前重试次数和退避策略配置，计算本次重试应等待的延迟时间。
%% 支持三种计算方式：
%% - constant: 固定延迟，始终为 initial_delay
%% - linear: 线性增长，delay = initial_delay * retry_count
%% - exponential: 指数增长，delay = initial_delay * multiplier^(retry_count-1)
%%
%% 计算结果不会超过 max_delay 配置的上限。
%%
%% @param RetryCount 当前重试次数（从 1 开始）
%% @param Config 退避策略配置映射
%% @returns 延迟时间（毫秒），正整数
-spec calculate_delay(pos_integer(), backoff_config()) -> pos_integer().
calculate_delay(RetryCount, #{type := Type,
                               initial_delay := InitialDelay,
                               max_delay := MaxDelay} = Config) ->
    Delay = case Type of
        constant -> InitialDelay;
        linear -> InitialDelay * RetryCount;
        exponential ->
            Multiplier = maps:get(multiplier, Config, 2),
            round(InitialDelay * math:pow(Multiplier, RetryCount - 1))
    end,
    min(Delay, MaxDelay).

%%====================================================================
%% 内部函数
%%====================================================================

%% @private 判断是否需要重试并执行重试逻辑
%% 检查当前工具的重试次数是否未超过上限，且错误类型是否可重试。
%% 如果满足重试条件：
%% 1. 计算退避延迟时间
%% 2. 调用重试回调函数（如已配置）
%% 3. 等待退避延迟（如已启用）
%% 4. 返回 {skip, RetryInfo} 信号通知上层进行重试
maybe_retry(Error, FilterCtx, MwState) ->
    #{max_retries := MaxRetries,
      backoff := BackoffConfig,
      on_retry := OnRetry,
      enable_delay := EnableDelay,
      retry_tracker := Tracker} = MwState,

    FuncName = case maps:get(function, FilterCtx, undefined) of
        #{name := N} -> N;
        _ -> <<"unknown">>
    end,

    RetryCount = maps:get(FuncName, Tracker, 0),
    CanRetry = RetryCount < MaxRetries andalso is_retryable(Error, MwState),

    case CanRetry of
        true ->
            NewCount = RetryCount + 1,
            Delay = calculate_delay(NewCount, BackoffConfig),

            %% 调用重试回调函数
            maybe_call_on_retry(OnRetry, FuncName, Error, NewCount, Delay),

            %% 等待退避延迟
            case EnableDelay andalso Delay > 0 of
                true -> timer:sleep(Delay);
                false -> ok
            end,

            %% 通过返回 skip 信号通知上层执行重试
            {skip, #{pending => retry, function => FuncName,
                     retry_count => NewCount, error => Error}};
        false ->
            %% 不可重试或已达最大重试次数，放行错误
            ok
    end.

%% @private 默认的可重试错误判断逻辑
%% - 当 retryable_errors 为 all 时，所有错误均可重试
%% - 当错误为 {ErrorType, _} 元组时，检查 ErrorType 是否在可重试列表中
%% - 当错误为单个原子时，直接检查是否在可重试列表中
%% - 其他格式的错误不可重试
is_retryable_default(_Error, all) -> true;
is_retryable_default({ErrorType, _}, RetryableErrors) when is_atom(ErrorType) ->
    lists:member(ErrorType, RetryableErrors);
is_retryable_default(ErrorType, RetryableErrors) when is_atom(ErrorType) ->
    lists:member(ErrorType, RetryableErrors);
is_retryable_default(_, _) -> false.

%% @private 安全调用重试回调函数
%% 如果 OnRetry 未定义则跳过，否则调用回调并捕获可能的异常。
%% 回调异常不会影响重试流程本身的执行。
maybe_call_on_retry(undefined, _, _, _, _) -> ok;
maybe_call_on_retry(OnRetry, ToolName, Error, RetryCount, Delay) when is_function(OnRetry, 4) ->
    try OnRetry(ToolName, Error, RetryCount, Delay)
    catch _:Reason -> logger:warning("Retry callback error: ~p", [Reason])
    end,
    ok.
