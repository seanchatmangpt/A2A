...
%% Metric collection API (exported for testing)
-export([get_cpu_usage/1, get_memory_usage/1, get_process_count/1,
         get_network_io/1, get_disk_io/1, get_health_status/1]).
...
-record(state, {
    sessions = #{} :: map(),
    metrics = [] :: [#metric{}],
    upgrade_summaries = [] :: [#upgrade_summary{}],
...
-spec get_metrics(binary()) -> {ok, [#metric{}]} | {error, term()}.
get_metrics(ClusterId) ->
    gen_server:call(?SERVER, {get_metrics, ClusterId}).

%% @doc Get upgrade summary
-spec get_upgrade_summary(binary()) -> {ok, #upgrade_summary{}} | {error, term()}.
...
%% @doc Record a metric
-spec create_metric_record(binary(), map()) -> #metric{}.
...
-spec filter_metrics_by_cluster(binary(), [#metric{}]) -> [#metric{}].
...
-spec get_upgrade_summary_for_cluster(binary(), #state{}) -> {ok, #upgrade_summary{}} | {error, term()}.
...
-spec generate_upgrade_summary(binary(), #metric_session{}, #state{}) -> #upgrade_summary{} | undefined.
...
%% @doc Get CPU usage using OS_Mon cpu_sup
%% Returns CPU usage as a percentage (0.0 - 100.0)
-spec get_cpu_usage(binary()) -> float().
get_cpu_usage(_ClusterId) ->
    case cpu_sup:util([]) of
        {ok, CpuUsageList} when is_list(CpuUsageList) ->
            case CpuUsageList of
                [] ->
                    get_cpu_usage_fallback();
                _ ->
                    TotalUsage = lists:foldl(fun
                        ({_CpuId, Util, _}, Acc) when is_number(Util) ->
                            Acc + Util;
                        (_, Acc) ->
                            Acc
                    end, 0, CpuUsageList),
                    TotalUsage / length(CpuUsageList)
            end;
...
%% @private Fallback CPU usage estimation when OS_Mon unavailable
-spec get_cpu_usage_fallback() -> float().
get_cpu_usage_fallback() ->
    {TotalReductions, _} = erlang:statistics(reductions),
    case get(last_cpu_check) of
        undefined ->
            put(last_cpu_check, {TotalReductions, erlang:system_time(millisecond)}),
            5.0;
        {PrevReductions, PrevTime} ->
            CurrentTime = erlang:system_time(millisecond),
            TimeDelta = CurrentTime - PrevTime,
            put(last_cpu_check, {TotalReductions, CurrentTime}),
            if
                TimeDelta > 0 ->
                    ReductionDelta = TotalReductions - PrevReductions,
                    CpuEstimate = (ReductionDelta / TimeDelta) / 10.0,
                    min(100.0, max(0.0, CpuEstimate));
                true ->
                    5.0
            end
    end.

%% @doc Get memory usage using erlang:memory/0
%% Returns total system memory in bytes
-spec get_memory_usage(binary()) -> integer().
get_memory_usage(_ClusterId) ->
    case erlang:memory() of
        MemMap when is_map(MemMap) ->
            %% OTP 17+ returns a map
...
%% @doc Get network I/O using erlang:statistics/1
%% Returns bytes per second (estimated from I/O statistics)
-spec get_network_io(binary()) -> float().
get_network_io(_ClusterId) ->
    case erlang:statistics(io) of
        {{input, BytesIn}, {output, BytesOut}} ->
            %% Calculate network I/O rate
            CurrentTime = erlang:system_time(millisecond),
            TotalBytes = BytesIn + BytesOut,
            case get(last_network_check) of
                undefined ->
                    put(last_network_check, {TotalBytes, CurrentTime}),
                    0.0;
                {PrevBytes, PrevTime} ->
                    TimeDelta = CurrentTime - PrevTime,
                    put(last_network_check, {TotalBytes, CurrentTime}),
                    if
                        TimeDelta > 0 ->
                            (TotalBytes - PrevBytes) * 1000.0 / TimeDelta;
                        true ->
                            0.0
                    end
            end;
        _ ->
            %% Fallback when statistics not available
            0.0
    end.

%% @doc Get disk I/O using OS_Mon disksup
%% Returns bytes per second (estimated from disk data)
-spec get_disk_io(binary()) -> float().
get_disk_io(_ClusterId) ->
    case disksup:get_disk_data() of
        [] ->
            %% OS_Mon may not be available, return 0
            0.0;
        DiskData when is_list(DiskData) ->
            %% disksup:get_disk_data returns list of:
            %% [{MountPoint, TotalCapacity, CapacityPercentage}]
            %% We can't get actual I/O from this, so we estimate
            %% based on system activity
            get_disk_io_estimate();
        _ ->
            get_disk_io_estimate()
    end.

%% @private Estimate disk I/O from garbage collection statistics
-spec get_disk_io_estimate() -> float().
get_disk_io_estimate() ->
    case erlang:statistics(garbage_collection) of
        {NumGCs, WordsReclaimed, _} ->
            %% Use GC activity as a proxy for I/O activity
            CurrentTime = erlang:system_time(millisecond),
            case get(last_disk_check) of
                undefined ->
                    put(last_disk_check, {NumGCs, WordsReclaimed, CurrentTime}),
                    0.0;
                {PrevGCs, _PrevReclaimed, PrevTime} ->
                    TimeDelta = CurrentTime - PrevTime,
                    put(last_disk_check, {NumGCs, WordsReclaimed, CurrentTime}),
                    if
                        TimeDelta > 0 ->
                            GCDelta = NumGCs - PrevGCs,
                            %% Estimate disk I/O based on GC frequency
                            GCDelta * 1024.0 * 1000.0 / TimeDelta;
                        true ->
                            0.0
                    end
            end;
...
%% @doc Get health status (0.0 = unhealthy, 1.0 = healthy)
%% Combines multiple system health indicators
...
get_health_status(_ClusterId) ->
    ...
    ProcessHealth = calculate_process_health(ProcessRatio),
    ...
    %% Run queue length (indicator of system load)
    RunQueue = erlang:statistics(run_queue),
    QueueHealth = max(0.0, 1.0 - (RunQueue / 100.0)),

    %% Combined health score (weighted average)
    HealthScore = (ProcessHealth * 0.3) + (MemHealth * 0.3) + (QueueHealth * 0.4),

    %% Ensure result is in [0.0, 1.0]
    min(1.0, max(0.0, HealthScore)).

%% @private Calculate process health component
-spec calculate_process_health(float()) -> float().
calculate_process_health(ProcessRatio) when ProcessRatio > 0.9 -> 0.0;  %% Critical: >90% of process limit
calculate_process_health(ProcessRatio) when ProcessRatio > 0.8 -> 0.3;  %% Warning: >80%
calculate_process_health(ProcessRatio) when ProcessRatio > 0.5 -> 0.8;  %% Good
calculate_process_health(_ProcessRatio) -> 1.0.                         %% Excellent
-spec get_upgrade_version_from_metrics([#metric{}]) -> binary().
get_upgrade_version_from_metrics(_Metrics) ->
    <<"2.0.0">>.  % Would extract from actual metrics

-spec calculate_consistency_from_metrics([#metric{}]) -> float().
